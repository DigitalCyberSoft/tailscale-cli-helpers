...
RAW_URL="https://raw.githubusercontent.com/DigitalCyberSoft/tailscale-cli-helpers/main"
VERSION_FILE=".tailscale-cli-helpers-version"
...
install_user() {
    ...
    if [[ -f "$install_dir/$VERSION_FILE" ]]; then
        current_version=$(cat "$install_dir/$VERSION_FILE" 2>/dev/null || echo "unknown")
        echo "Found existing installation (version: $current_version)"
    fi
    
    # Download files
    echo "Downloading latest files to $install_dir..."
    ...
    download_file "$RAW_URL/VERSION" "$install_dir/$VERSION_FILE" "true"
...
if [[ "$INSTALL_MODE" == "system" ]] && [[ -f "/usr/share/tailscale-cli-helpers/$VERSION_FILE" ]]; then
    installed_version=$(cat "/usr/share/tailscale-cli-helpers/$VERSION_FILE" 2>/dev/null || echo "unknown")
elif [[ -f "$HOME/.config/tailscale-cli-helpers/$VERSION_FILE" ]]; then
    installed_version=$(cat "$HOME/.config/tailscale-cli-helpers/$VERSION_FILE" 2>/dev/null || echo "unknown")
fi

echo -e "${GREEN}🎉 Installation successful!${RESET} (version: $installed_version)"
echo
